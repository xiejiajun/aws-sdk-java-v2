...
public class FailureUtils {
    private static Logger log = Logger.loggerFor(FailureUtils.class);

    public static void runAndLogIfFails(Runnable thingToRun, Supplier<String> logOnFailure) {
        try {
            thingToRun.run();
        } catch (Throwable e) {
            log.error(logOnFailure, e);
        }
    }

    public static void failRequestFuture(Channel channel, Throwable cause) {
        CompletableFuture<Void> future = channel.attr(ChannelAttributeKey.EXECUTE_FUTURE_KEY).get();
        Validate.notNull(future, "Execute future must not be null.");
        failRequestFuture(future, cause);
    }

    public static void failRequestFuture(CompletableFuture<?> future, Throwable cause) {
