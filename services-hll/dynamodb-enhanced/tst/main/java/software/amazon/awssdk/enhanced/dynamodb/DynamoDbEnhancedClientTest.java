...
public class DynamoDbEnhancedClientTest {
    private static final DynamoDbClient GENERATED_CLIENT = Mockito.mock(DynamoDbClient.class);
    ...
    public void tableNameIsCorrect() {
        Table table = CLIENT.table("table");
    ...
    public void putItemSendsCorrectValues() {
        ...
        Mockito.when(GENERATED_CLIENT.putItem(putRequestCaptor.capture())).thenReturn(PutItemResponse.builder().build());
    ...
    public void getItemSendsCorrectValues() {
        ...
        Mockito.when(GENERATED_CLIENT.getItem(getRequestCaptor.capture())).thenReturn(generatedResponse);

        Table table = CLIENT.table("table");
        ResponseItem responseItem = table.getItem(r -> r.putAttribute("foo", "bar"));
    ...
    public void closeDoesNotCloseUnderlyingClientWhenProvidedByCustomer() {
        DynamoDbEnhancedClient.builder().dynamoDbClient(GENERATED_CLIENT).build().close();
