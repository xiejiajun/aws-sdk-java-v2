...
public final class ChainAttributeConverter<T> implements SubtypeAttributeConverter<T> {
    ...
    private ChainAttributeConverter(Builder<T> builder) {
        ...
        for (int i = builder.converters.size() - 1; i >= 0; i--) {
            ...
            converterCache.put(converter.type().rawClass(), new AttributeConverterUnion<>(converter));
        }
    }
    ...
    public ItemAttributeValue toAttributeValue(T input, ConversionContext context) {
        ...
        return findRequiredConverter((Class<T>) input.getClass()).toAttributeValue(input, context);
    }
    ...
    private static class AttributeConverterUnion<U> {
        ...
        private ItemAttributeValue toAttributeValue(U input, ConversionContext context) {
            if (converter != null) {
                return converter.toAttributeValue(input, context);
            } else if (subtypeConverter != null) {
                return subtypeConverter.toAttributeValue(input, context);
            } else {
                throw new IllegalStateException();
            }
        }

        private U fromAttributeValue(ItemAttributeValue input, TypeToken<U> targetType, ConversionContext context) {
            if (converter != null) {
                return converter.fromAttributeValue(input, context);
            } else if (subtypeConverter != null) {
                return subtypeConverter.fromAttributeValue(input, targetType, context);
            } else {
                throw new IllegalStateException();
            }
        }
